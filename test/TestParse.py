...
class TestParse(unittest.TestCase):
    def test_get_model_list(self):
        ...
        with open("test.esdl", mode="r") as esdl_file:
            encoded_base64_esdl = base64.b64encode(
                esdl_file.read().encode("utf-8")
            ).decode("utf-8")

        calculation_service_energy_system = CalculationService()
        calculation_service_energy_system.esdl_type = "EnergySystem"
        calculation_service_energy_system.calc_service_name = (
            "test2_energy_system_service"
        )
        calculation_service_energy_system.service_image_url = (
            "dotsenergyframework/test2_energy_system_service:0.0.1"
        )
        ...
        calculation_service_pv_installation.calc_service_name = (
            "test2_pv_installation_service"
        )
        calculation_service_pv_installation.service_image_url = (
            "dotsenergyframework/test2_pv_installation_service:0.0.1"
        )
        ...
        calculation_service_e_connection.service_image_url = (
            "dotsenergyframework/test2_econnection_service:0.0.1"
        )
        ...
        expected_energysystem_ids = ["37e0284d-29f8-453e-91ec-2e7e02b5c16d"]
        ...
        self.assertEqual(
            sum(
                [
                    calculation_service_e_connection.nr_of_models,
                    calculation_service_pv_installation.nr_of_models,
                    calculation_service_energy_system.nr_of_models,
                ]
            ),
            len(model_list),
        )
        energy_system_models = [
            model
            for model in model_list
            if model.calc_service.calc_service_name
            == calculation_service_energy_system.calc_service_name
        ]
        pv_installation_models = [
            model
            for model in model_list
            if model.calc_service.calc_service_name
            == calculation_service_pv_installation.calc_service_name
        ...
        e_connection_models = [
            ...
            for model in model_list
            if model.calc_service.calc_service_name
            == calculation_service_e_connection.calc_service_name
        ]
        ...
        self.assertListEqual(
            expected_energysystem_ids, energy_system_models[0].esdl_ids
        )
        self.assertListEqual(
            expected_pv_installation_ids,
            first_pv_installation_model.esdl_ids
            + second_pv_installation_model.esdl_ids,
        )

        # Assert correct amount of models
        self.assertEqual(
            calculation_service_energy_system.nr_of_models, len(energy_system_models)
        )
        self.assertEqual(
            calculation_service_pv_installation.nr_of_models,
            len(pv_installation_models),
        )
        self.assertEqual(
            calculation_service_e_connection.nr_of_models, len(e_connection_models)
        )
