...
class DataHandler:
    ...
    def get_simulation_results(self, measurement: str, simulation_id: str) -> pd.DataFrame:
        query = f"SELECT * FROM \"{measurement}\" WHERE simulation_id = \'{simulation_id}\'"
    ...
    def get_all_data_for_simulation_id(self, simulation_id: str) -> BytesIO:
        ...
        zip_buffer = BytesIO()
        has_data = False
        with zipfile.ZipFile(file=zip_buffer, mode="w", compression=zipfile.ZIP_DEFLATED, compresslevel=9) as zip_archive:
            for measurement in result_set.get_points():
                measurement_name = measurement["name"]
                measurement_dataframe = self.get_simulation_results(measurement_name, simulation_id)
